    ...
    $.post("/Project/goods",{"type":"getGoodsKeyId","key":key},function (data) {
        ...
        if (data.info.length>0){
            data.info.forEach(function (item) {
                url = item.url.split(",");
                // console.log(url[0]);
                html+="<div class=\"silder-main-img\">"
                html+="<a class=\"images\" data-id=\""+item.goods_id+"\" href=\"/Project/goods?type=getGoodsById&goods_id="+item.goods_id+"\"><img src=\"/Project/images/head/"+url[0]+"\"></a>"
                html+="</div>"
            })
        }
        $("#mainCar").empty().append(html);
    },"json")
}

function getGoodsUlrFt() {
    let key = $("#FT").data("id");
    $.post("/Project/goods",{"type":"getGoodsKeyId","key":key},function (data) {
        // console.log(data);
        let html = "";
        ...
        $("#windUlr").empty().append(html);
    },"json")
}

function getGoodsUlrFs() {
    let key = $("#FS").data("id");
    $.post("/Project/goods",{"type":"getGoodsKeyId","key":key},function (data) {
        // console.log(data);
        let html = "";
        ...
        $("#mianFs").empty().append(html);
    },"json")
}

function getGoodsUlr() {
    let key = $("#CX").data("id");
    $.post("/Project/goods",{"type":"getGoodsKeyId","key":key},function (data) {
        // console.log(data);
        let html = "";
        ...
        $("#maincx").empty().append(html);
    },"json")
}

function getGoodsCar() {
    let key = $("#Car").data("id");
    $.get("/Project/goods",{"type":"getGoodsKeyId","key":key},function (data) {
        // console.log(data);
        let html = "";
        let url = "";
        if (data.info.length>0){
            data.info.forEach(function (item) {
                url = item.url.split(",");
                html+="<div class=\"you_shangp_lieb \">"
                html+="<a class=\"images\" data-id=\""+item.goods_id+"\" href=\"/Project/goods?type=getGoodsById&goods_id="+item.goods_id+"\"><img class=\"you_tup_k\" src=\"/Project/images/head/"+url[0]+"\"></a>"
                html+="<a class=\"_you_neir_biaot\">"+item.goods_name+"</a>"
                html+="<span>¥"+item.price+"</span>"
                html+="</div>"
            })
        }
        $("#goodsCar").empty().append(html);
    },"json")
}
...
function getGoodsoff() {
    ...
    $.get("/Project/goods",{"type":"getGoodsKeyId","key":key},function (data) {
        ...
        $("#officeFurniture").empty().append(html);
    },"json")
}
